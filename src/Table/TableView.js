...
function TableView({ id, dailyRunningTime, endDay, startDay, tripLists }) {
  ...
  // Toggle expand and collapse table
  ...
  useEffect(() => {
    // calculating overall total expense and total km
  ...
  return (
    <div className="TableView">
      <div className="collapsible active" onClick={expandTable}>
        <span>
          Date: {getDateTime(startDay)} - {getDateTime(endDay)} (
          {getTimeDiff(endDay, startDay, 1)})
        </span>
        <div className="table__totalExpenseskm">
          <span className="table__totalKM">Total KM: {totalKm} KM</span>
          <span className="table__totalExpenses">
            Total Expenses: {totalExpSum}
          </span>
        </div>
