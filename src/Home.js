...
// calculate diffrence of two time
...
function Home() {
  ...
  const arrayItem = item?.data?.tripDetails;
      ...
      {arrayItem?.map((val) => {
        return (
          <TableView
            key={val?.id}
            id={val?.id}
            dailyRunningTime={val?.dailyRunningTime}
            endDay={val?.endDay}
            startDay={val?.startDay}
            tripLists={val?.tripLists}
          />
        );
      })}
    </div>
